...
with col1:
    ...
    if st.button("Signup",use_container_width = True, type = "primary"):
        if name == "" and email == "" and password == "" and company_name == "" and contact_number == "":
            ...
            try:
                ...
                cursor.execute("INSERT INTO employer(user_name, name, email, password,  contact_no, skills) VALUES (?, ?, ?, ?, ?, ?)", (name, name, email, password, contact_number, ""))
                connection.commit()
    ...
    st.page_link("pages/employee_login.py",label = "Already have an account")
